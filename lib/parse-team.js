...
const parseTeam = ({ body, baseTeam }) => {
  ...
  for (const playerRow of teamTableRef.children()) {
    ...
    const transfermarktId = playerUrl.split('/').at(-1)
  ...
  return {
    allPlayers: allPlayers.map(p => {
      const { name, transfermarktId, appearances } = p
      return { name, transfermarktId, appearances }
    }),
    boardPlayers,
    ...baseTeam
  }
...
const finalizeTeam = ({ allPlayers, position }) => {
  ...
  if (!boardPlayers.find(p => bestPlayers[0].transfermarktId === p.transfermarktId)) {
